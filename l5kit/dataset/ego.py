...
None if not desired
        ...
        self.base_rasterizer = base_rasterizer
        ...
        self.sample_function = partial(
            ...
            rasterizer=rasterizer,
            base_rasterizer=base_rasterizer,
    ...
    def get_frame(self, scene_index: int, state_index: int, track_id: Optional[int] = None) -> dict:
        ...
        # 0,1,C -> C,0,1
        scene_image = data["scene_image"].transpose(2, 0, 1)
        base_image = data["base_image"].transpose(2, 0, 1)
        scene_ego_image = data["scene_ego_image"].transpose(2, 0, 1)
        scene_agents_image = data["scene_agents_image"].transpose(2, 0, 1)
        ego_agents_image = data["ego_agents_image"].transpose(2, 0, 1)
        ...
        return {
            ...
            "path":data["path"],
            "final_lane":data["final_lane"],
            "final_lane_size":data["final_lane_size"],
            "final_lane_num":data["final_lane_num"],
    ...
    def get_scene_dataset(self, scene_index: int) -> "EgoDataset":
        ...
        return EgoDataset(self.cfg, dataset, self.rasterizer, self.perturbation,base_rasterizer = self.base_rasterizer,)
