...
def update_mask(mask: np.ndarray, agent_list: list) -> None:
    ...
    for idx_el, (frame_idx, mask_idx, agent) in enumerate(agent_list):
        max_x=max(agent["centroid"][0],max_x)
        max_y=max(agent["centroid"][1],max_y)
        min_x=min(agent["centroid"][0],min_x)        
        min_y=min(agent["centroid"][1],min_y)
    if abs(max_x-min_x)<1 and abs(max_y-min_y)<1:
        filter_coef=0
    for idx_el, (frame_idx, mask_idx, agent) in enumerate(agent_list):
        mask[mask_idx][0] = idx_el*filter_coef  # past information
        mask[mask_idx][1] = (len(agent_list) - idx_el - 1)*filter_coef  # future information
