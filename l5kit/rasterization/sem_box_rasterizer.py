...
from .my_semantic_rasterizer import MySemanticRasterizer
...
class SemBoxRasterizer(Rasterizer):
    ...
    def __init__(
        ...
        self.sat_rast = MySemanticRasterizer(raster_size, pixel_size, ego_center, semantic_map_path, world_to_ecef)
...
#         im_out_box_ego,im_out_box_agents,im_out_box = self.box_rast.rasterize(history_frames, history_agents, history_tl_faces, agent)
        im_out_sat = self.sat_rast.rasterize(history_frames, history_agents, history_tl_faces, agent)
#         return im_out_sat,im_out_box_ego,im_out_box_agents
        return np.concatenate([im_out_sat, im_out_sat], -1),np.concatenate([im_out_sat, im_out_sat], -1),np.concatenate([im_out_sat, im_out_sat], -1),im_out_sat,np.concatenate([im_out_sat, im_out_sat], -1)
