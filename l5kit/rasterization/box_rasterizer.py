...
from ..geometry import rotation33_as_yaw, transform_points, world_to_image_pixels_matrix,transform_point
...
#     print(agents.shape)
#     print(agents)
#     print(viewable_agents_trak_id)
    
    if viewable_agents_trak_id is None:
        tmp_agents=[]

        for idx, agent in enumerate(agents):
            reletive_position=agent["centroid"]-target["centroid"]
            normal_position=transform_point(reletive_position,yaw_as_rotation33(-target["yaw"]))

            if np.count_nonzero(reletive_position)==0:
                tmp_agents.append(agent)
            else:
                if np.linalg.norm(reletive_position)<MAX_VIEW_DIST and normal_position[0]>=-2 :
                    tmp_agents.append(agent)

        agents=np.array(tmp_agents)
    else:
        ...
        for idx, agent in enumerate(agents):
            ...
            if agent["track_id"] in viewable_agents_trak_id:
                tmp_agents.append(agent)
...
class BoxRasterizer(Rasterizer):
    ...
    def satisfy_constraints(self,agents,target):
        track_ids=[]
        ...
        for idx, agent in enumerate(agents):
            ...
            if np.count_nonzero(reletive_position)==0:
                track_ids.append(agent["track_id"])
            else:
                if np.linalg.norm(reletive_position)<MAX_VIEW_DIST and normal_position[0]>=-2:
                    track_ids.append(agent["track_id"])
        return track_ids
    
    def find_in_view_agents(self,frame, agents,agent):
        ...
        if agent is None:
            return self.satisfy_constraints(agents,av_agent[0])
        else:
            ...
            if len(agent_ego) != 0:  # agent not in this history frame
                ...
                return self.satisfy_constraints(np.append(agents, av_agent),agent_ego[0])
        return None
...
#         viewable_agents_trak_id=self.find_in_view_agents(history_frames[0],history_agents[0],agent)
        viewable_agents_trak_id=None
        for i, (frame, agents) in enumerate(zip(history_frames, history_agents)):
            agents = filter_agents_by_labels(agents, self.filter_agents_threshold)
            # note the cast is for legacy support of dataset before April 2020
            av_agent = get_ego_as_agent(frame).astype(agents.dtype)
            if agent is None:
                agents_image = draw_boxes(self.raster_size, world_to_image_space, agents, 255,av_agent[0],viewable_agents_trak_id)
                ego_image = draw_boxes(self.raster_size, world_to_image_space, av_agent, 255,av_agent[0],viewable_agents_trak_id)
            else:
                agent_ego = filter_agents_by_track_id(agents, agent["track_id"])
                if len(agent_ego) == 0:  # agent not in this history frame
                    continue
...
#                     ego_image = np.zeros_like(agents_image)
                    ...
                    agents = agents[agents != agent_ego[0]]
                    agents_image = draw_boxes(self.raster_size, world_to_image_space, np.append(agents, av_agent), 255,agent_ego[0],
                                             viewable_agents_trak_id)
                    ego_image = draw_boxes(self.raster_size, world_to_image_space, agent_ego, 255,agent_ego[0],
                                          viewable_agents_trak_id)
        ...
        return ego_images.astype(np.float32) / 255,agents_images.astype(np.float32) / 255,out_im.astype(np.float32) / 255
