...
class Encoder(nn.Module):
    def __init__(self, cfg: _DefaultConfig,mlp=None):
        super().__init__()

        self.cfg = cfg
        self.history_mlp = mlp
        ...
        if cfg.encode_stages == 2:
            if not cfg.self_match:
                ...
                self.hierarchical_PE = PositionalEncodingLUT(cfg.d_model, max_len=cfg.max_num_groups+1)
    ...
    def forward(self, commands, args, history, label=None):
        ...
        h = self.history_mlp_l(self.history_mlp(torch.flatten(history, start_dim=1))).unsqueeze(0).unsqueeze(0)
        # print(h.shape)
...
class SVGTransformer(nn.Module):
    def __init__(self, cfg: _DefaultConfig,mlp=None):
        ...
        if self.cfg.encode_stages > 0:
            ...
            self.encoder = Encoder(cfg,self.history_mlp)
            if cfg.use_resnet:
