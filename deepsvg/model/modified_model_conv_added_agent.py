...
class Encoder(nn.Module):
    def __init__(self, cfg: _DefaultConfig):
        ...
        self.cfg = cfg
        self.history_residual = torch.nn.Sequential(torch.nn.Conv1d(in_channels=1,out_channels=8,kernel_size=4,stride=2,padding=0),
                                                    torch.nn.ReLU(),
                                                    torch.nn.Conv1d(in_channels=8,out_channels=32,kernel_size=4,stride=2,padding=0),)
        self.agent_residual = torch.nn.Sequential(torch.nn.Conv1d(in_channels=1,out_channels=8,kernel_size=4,stride=2,padding=0),
                                                  torch.nn.ReLU(),
                                                  torch.nn.Conv1d(in_channels=8,out_channels=32,kernel_size=4,stride=2,padding=0),)
        ...
        if cfg.encode_stages == 2:
            if not cfg.self_match:
                ...
                self.hierarchical_PE = PositionalEncodingLUT(cfg.d_model, max_len=cfg.max_num_groups+1+10)
    ...
    def forward(self, commands, args, history,agents,agents_validity,label=None):
        ...
        if self.cfg.encode_stages == 2:
            visibility_mask, key_visibility_mask = _get_visibility_mask(commands, seq_dim=0,
                                                                        modified=True,agents_validity= agents_validity), _get_key_visibility_mask(
                commands, seq_dim=0, modified=True,agents_validity= agents_validity)
        ...
        b = history.shape[0]
        h = self.history_residual(torch.flatten(history,start_dim=1).unsqueeze(1)).reshape((b,-1)).unsqueeze(0).unsqueeze(0)
        agents=agents.permute(1,0,2,3)
        z = torch.cat((z, h), dim=1)
        for agent in agents:
            a = self.agent_residual(torch.flatten(agent.type(torch.cuda.FloatTensor),
                                                  start_dim=1).unsqueeze(1)).reshape((b,-1)).unsqueeze(0).unsqueeze(0)
...
class SVGTransformer(nn.Module):
    ...
    def forward(self, commands_enc, args_enc, commands_dec, args_dec, history,agents,agents_validity, label=None,
