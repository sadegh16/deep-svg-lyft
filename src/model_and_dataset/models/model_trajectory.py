...
class ModelTrajectory(RasterModel):
    def __init__(self, model_cfg, data_config, modes=1, future_len=None, in_channels=None, pretrained=True,model_type = None):
        ...
        if model_type == 1:
            ...
            self.model = SVGTransformer(self.model_cfg.model_cfg)
        if model_type == 6:
            ##TODO Import Right SVGTransformer Vahid
            self.model = SVGTransformer(self.model_cfg.model_cfg)
        if model_type == 7:
            from deepsvg.model.modified_model_lstm_between import SVGTransformer
            self.model = SVGTransformer(self.model_cfg.model_cfg)



    def _forward(self, x):
        if self.model_type == 4 or self.model_type == 5 or self.model_type == 7:
