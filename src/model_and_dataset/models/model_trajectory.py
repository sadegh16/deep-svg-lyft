...
class ModelTrajectory(RasterModel):
    def __init__(self, model_cfg, data_config, modes=1, future_len=None, in_channels=None, pretrained=True,
        ...
        if model_type == 1:
            ...
            self.model = SVGTransformer(self.model_cfg.model_cfg)
        if model_type == 6:
            from deepsvg.model.sep_encoder import SVGTransformer
            self.model = SVGTransformer(self.model_cfg.model_cfg)
    ...
    def _forward(self, x):
        ...
        if self.model_type == 6:
            ...
            return self.model(commands_enc, args_enc, commands_dec, args_dec, commands_his=commands_his,
                              args_his=args_his, params=params,
                              encode_mode=encode_mode)
