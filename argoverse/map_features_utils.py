...
#         fig=plt.figure(figsize=(20, 20))
        ...
        if viz:
            img = 255 * np.ones(shape=(224, 224, 3), dtype=np.uint8)

#             plt.figure(0, figsize=(8, 7))
            for centerline_coords in candidate_centerlines:
                cnt_line = cv2_subpixel(transform_points(centerline_coords, world_to_image_space))
                cv2.polylines(img, [cnt_line], False, (0, 0, 0), lineType=cv2.LINE_AA,shift=CV2_SHIFT )
...
#             plt.show()
        ...
        for centerline_coords in candidate_centerlines:
            candidate_centerlines_normalized.append(transform_points(centerline_coords-centroid, yaw_as_rotation33(-ego_yaw)))
...
#             plt.show()
        ...
        return candidate_centerlines,img,candidate_centerlines_normalized
